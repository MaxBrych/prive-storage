...
export const FundWithdraw: React.FC<FundWithdrawConfigProps> = ({
  ...
  const initialSelectedNode = node
    ? nodes.find((n) => n.value === node) || null
    : null;
  const initialSelectedCurrency = currency
  ...
  const [selectedNode, setSelectedNode] = useState<OptionType | null>(
    initialSelectedNode
  );
  ...
  const handleOptionChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setIsFunding(e.target.value === "fund");
  ...
  useEffect(() => {
    ...
    const getCurBalance = async () => {
      try {
        ...
        if (!isFunding)
          setAmount(irys.utils.fromAtomic(loadedBalance).toString());
      } catch (error) {
    ...
    if (isFunding) {
      try {
        const fundTx = await irys.fund(
          irys.utils.toAtomic(new BigNumber(amount))
        );
      ...
      try {
        const fundTx = await irys.withdrawBalance(
          irys.utils.toAtomic(new BigNumber(amount))
        );
  ...
  return (
    <div className="w-full p-5 bg-white border rounded-xl border-zinc-100 ">
      {!node && (
        <Select
          className="mb-4"
          options={nodes}
      ...
      <input
        type="number"
        step="0.0000001"
        className="block w-full p-3 mb-4 bg-transparent border border-gray-300 rounded-md shadow-sm text-text"
        value={amount}
        onChange={handleAmountChange}
      />
      {!fundOnly && !withdrawOnly && (
        <div className="flex items-center my-6 space-x-4 text-text">
